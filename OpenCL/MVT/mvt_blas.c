...
double cl_blas(){
	...
	t_start = rtclock();
	  ...
	  // x1  = x1 +  (A * y1)
        err = clAmdBlasSgemvEx(order, notransA, N, N, 1.0, a_mem_obj, 0, N,
		 y1_mem_obj, 0, inc,
		1.0, x1_mem_obj, 0, inc, 
	...
	if (err != CL_SUCCESS) {
	  ...
	  // x2  = x2 +  (A` * y2)
        err = clAmdBlasSgemvEx(order, transA, N, N, 1.0, a_mem_obj, 0, N,
		y2_mem_obj, 0, inc,
		1.0, x2_mem_obj, 0, inc, 
		1, &clCommandQue, 0, NULL, &event);
	...
	t_end = rtclock(); 
	fprintf(stdout, "kernel execution: %0.6lfs\n", t_end - t_start);   
...
int main(void) 
{
	...
	fprintf(stdout, "copy + kernel : %0.6lfs\n", t_copy + t_kernel);   
