...
#define PERCENT_DIFF_ERROR_THRESHOLD 0.3
...
void init_array(DATA_TYPE *x, DATA_TYPE *A)
{
	...
	for (i = 0; i < NX; i++)
	{
		...
		for (j = 0; j < NY; j++)
		{
			A[IDX2C(i, j, NY)] = ((DATA_TYPE) i*(j)) / NX;
		}
	}
}
	...
	for (i = 0; i < NX; i++)
 	{
      	...
      	for (j = 0; j < NY; j++)
		{
			tmp[i] = tmp[i] + A[IDX2C(i, j, NY)] * x[j];
		}
		
      	for (j = 0; j < NY; j++)
		{
			y[j] = y[j] + A[IDX2C(i, j, NY)] * tmp[i];
		}
    }
}
...
void ataxGpu(DATA_TYPE* A, DATA_TYPE* x, DATA_TYPE* y, DATA_TYPE* tmp, DATA_TYPE* y_outputFromGpu)
{
	...
	double t_start_k = rtclock();
	stat = cublasSgemv(handle, CUBLAS_OP_N, NX, NY, &alpha, A_gpu, 
		NX, x_gpu, 1, &beta, tmp_gpu, 1);
	if(stat != CUBLAS_STATUS_SUCCESS){
		printf("Error in culbas sgemv\n");
		return;
	}

	stat = cublasSgemv(handle, CUBLAS_OP_T, NX, NY, &alpha, A_gpu, 
		NX, tmp_gpu, 1, &beta, y_gpu, 1);
	if(stat != CUBLAS_STATUS_SUCCESS){
		printf("Error in culbas sgemv\n");
		return;
	}
...
int main(int argc, char** argv)
{
	...
	t_end = rtclock();
	fprintf(stdout, "BLAS Complete  Runtime: %0.6lf\n", t_end - t_start);
